...
class Lexer {
    ...
    consumeText(text) {
        ...
        while (p--) if (this.source.charCodeAt(this.cursor + p) !== text.charCodeAt(p)) {
            return false
        }
...
class Parser extends Lexer {
    ...
    primary() {
        ...
        do switch (cc = this.advance()) {
            ...
            case FullStop$1:
                this.cursor++;
                primary = {type: AST.MemberExpression, object: primary, property: this.identifier(true), computed: false};
                continue;
            default:
                return primary;
        } while (true);
...
/**
 * This is a constructor for asynchronous functions same as new Function(...).
 *
...
function expressionMixin(BaseFunction) {
    ...
    return class extends BaseFunction {
        ...
        constructor(parameters, code, sourceURL) {
            super(...parameters, [
                '//# sourceURL=' + sourceURL,
                'return ' + code
            ].join('\n'));
        ...
        toString() {
            return `f (${this.parameters.join(', ')}) { //# sourceURL=${this.sourceURL} ... }`;
        }
    }
}

const Expression = expressionMixin(Function);
const AsyncExpression = expressionMixin(AsyncFunction);
...
class Compiler {
    ...
    static compile(source, sourceURL = `expression?id=${Compiler.uniqueID++}`) {

        const ast = new Parser(source).ast();

        const {code, parameters, isAsync} = new Visitor().visit(ast);

        return isAsync ? new AsyncExpression(parameters, code, sourceURL): new Expression(parameters, code, sourceURL);
    }
}
