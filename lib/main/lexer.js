...
let Lexer = exports.Lexer = class Lexer {
    ...
    advance(offset) {
        ...
        let cc,
            position = offset ? this.position + offset : this.position;
    ...
    nextLiteral() {
        ...
        while ((cc = charCodeAt(++position)) && (cc >= LetterA && cc <= LetterZ || cc >= CapitalA && cc <= CapitalZ || cc >= DigitZero && cc <= DigitNine || cc === LowLine || cc === DollarSign));
    ...
    debug(length) {
        let from = this.position,
            to = Math.min(from + length, this.source.length);
